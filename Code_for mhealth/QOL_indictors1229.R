...
# pick variables
dfall.qol <- df.all %>% 
  dplyr::select(q1_id, number, group, q3_gender, q4_birth, date, q8_huji, 
         q9_education, q10_sexori, q11_marriage, q12_child, q13_dusheng, q15_job, 
...
fit.qol %>% plot()
# 综合评价
fit.qol %>% gvlma::gvlma.lm() %>% summary()
# 标化系数
lm.beta(fit.qol)
...
df.item <- df.all %>% dplyr::select(starts_with('qoldomain'), ends_with('cope'), starts_with('q68_'), cesdscore)
names(df.item)
write.table(df.item, 'sem_qol.txt', sep = '\t',col.names = FALSE, row.names = FALSE)

ggplot(dfall.qol, aes(cesdscore, qolscore)) + geom_point()
